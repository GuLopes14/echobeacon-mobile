...
// options can be an mqtt connect URL (string) or mqtt.IClientOptions
const useMqttClient = (options?: mqtt.IClientOptions | string): useMqttClientProps => {
  ...
  useEffect(() => {
    ...
    // If options is an object, require it to have at least one key to avoid connecting with an empty object
    if (typeof options === "object" && Object.keys(options).length === 0) return;

    // mqtt.connect accepts either a URL string or an options object
    const mqttClient = typeof options === "string" ? mqtt.connect(options) : mqtt.connect(options);
